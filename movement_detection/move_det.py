...
def main(path):
    ...
    while True:
        ...
        for box_id in boxer_ids:
            ...
            cv2.putText(frame, "Obj" + str(id), (x, y - 15), cv2.FONT_ITALIC, 0.7, (255, 0, 0), 2)
            cv2.rectangle(frame, (x, y), (x + w, y + h), (0, 0, 255), 2)  # 根据移动物体的轮廓添加boundingbox

            # 然后每检测出一个预测框，就将中心点加入队列
            if id in pts:
                pts[id].append(center1)
            else:
                pts[id] = []
                pts[id].append(center1)

            thickness = 2
            cv2.circle(frame, (center1), 1, [255, 255, 255], thickness)

            for j in range(1, len(pts[id])):
                if pts[id][j - 1] is None or pts[id][j] is None:
                    continue
                cv2.line(frame, (pts[id][j - 1]), (pts[id][j]), [0, 0, 255], thickness)
