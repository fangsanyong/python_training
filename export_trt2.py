...
def export_engine(model, im, file, train, half, simplify, workspace=4, verbose=False, prefix=colorstr('TensorRT:')):
    ...
    try:
        ...
        bs, num_boxes, cc = previous_output.shape
        clss = cc - 5 
        ...
        # indices = network.add_constant(trt.Dims([clss]), trt.Weights(np.zeros(clss, np.int32)))
        ...
        scores = network.add_elementwise(obj_score.get_output(0), scores.get_output(0), trt.ElementWiseOperation.PROD)

        # reshape box to [bs, num_boxes, 1, 4]
        reshaped_boxes = network.add_shuffle(boxes.get_output(0))
        reshaped_boxes.reshape_dims = trt.Dims([bs, num_boxes, 1, 4])
        ...
        assert (creator)
        fc = []
        fc.append(trt.PluginField("shareLocation", np.array([1], dtype=np.int32), trt.PluginFieldType.INT32))
        fc.append(trt.PluginField("backgroundLabelId", np.array([-1], dtype=np.int32), trt.PluginFieldType.INT32))
        fc.append(trt.PluginField("numClasses", np.array([clss], dtype=np.int32), trt.PluginFieldType.INT32))
        fc.append(trt.PluginField("topK", np.array([100], dtype=np.int32), trt.PluginFieldType.INT32))
        fc.append(trt.PluginField("keepTopK", np.array([100], dtype=np.int32), trt.PluginFieldType.INT32))
        fc.append(
            trt.PluginField("scoreThreshold", np.array([0.2], dtype=np.float32), trt.PluginFieldType.FLOAT32))
        fc.append(trt.PluginField("iouThreshold", np.array([0.45], dtype=np.float32), trt.PluginFieldType.FLOAT32))
        fc.append(trt.PluginField("isNormalized", np.array([0], dtype=np.int32), trt.PluginFieldType.INT32))
        fc.append(trt.PluginField("clipBoxes", np.array([0], dtype=np.int32), trt.PluginFieldType.INT32))

        fc = trt.PluginFieldCollection(fc)
        nms_layer = creator.create_plugin("nms_layer", fc)

        layer = network.add_plugin_v2([reshaped_boxes.get_output(0), scores.get_output(0)], nms_layer)
        ...
        # fc.append(trt.PluginField("max_output_boxes", np.array([100], dtype=np.int32), trt.PluginFieldType.INT32))
        # fc.append(trt.PluginField("score_threshold", np.array([0.2], dtype=np.float32), trt.PluginFieldType.FLOAT32))
        # fc.append(trt.PluginField("iou_threshold", np.array([0.45], dtype=np.float32), trt.PluginFieldType.FLOAT32))
...
def parse_opt():
    ...
    parser.add_argument('--data', type=str, default=ROOT / 'data/mix.yaml', help='dataset.yaml path')
    parser.add_argument('--weights', nargs='+', type=str, default=ROOT / 'runs/train/mix12/weights/best.pt', help='model.pt path(s)')
    parser.add_argument('--imgsz', '--img', '--img-size', nargs='+', type=int, default=[512, 640], help='image (h, w)')
    parser.add_argument('--batch-size', type=int, default=1, help='batch size')
    parser.add_argument('--device', default='1', help='cuda device, i.e. 0 or 0,1,2,3 or cpu')
    parser.add_argument('--half', action='store_true',default=True, help='FP16 half-precision export')
