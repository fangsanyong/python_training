...
from utils.pb import PhotometricDistort
# Parameters
...
class LoadImagesAndLabels(Dataset):
    ...
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        self.pb = PhotometricDistort() if augment else None
    ...
    def __getitem__(self, index):
        ...
        if self.augment and nl == 0 and random.random() < hyp['mixup']:
            img, labels = mixup(img, labels, *self.aa(random.randint(0, self.n - 1)))
        nl = len(labels) 
        ...
        if self.augment:
            ...
            img1 = self.pb(img.astype(np.float32))
            img = img1.astype(np.uint8).copy()
...
def verify_image_label(args):
    ...
    try:
        ...
        if os.path.isfile(lb_file) :
            ...
            # 人和鸟的标签都是0 修改人画面里面只保留人的标签  鸟的标签修改为1   
            # nl = len(lb)
            # if lb_file.find('/bird/') <0:
            #      if nl:
            #          # coco数据里里面有动物目标 防止鸟的影响训练结果直接过滤掉
            #          inc_5 = (lb[:,0]==4)
            #          ot = lb[inc_5]
            #          if len(ot):
            #             nc = 1
